...
for(row in input){
    input[row] = input[row].split(",").join().trim( ).split(",");
...
function sortArray() {
    for(row in input){
        input[row].sort(function(a, b){return a - b});
    }
}
...
for(row in input){
    ...
    adverge.push(max - min);
    //input[row].push(input[row]);
}
console.log(sum);

add = function(adverge) {
    return adverge.reduce((a,b) => a + b, 0);
}

let satTotalp2 = add(adverge);


//console.table(adverge);
//console.log(satTotalp2);
...
//for(let i = 0; i < 9; i++){
    //for(row in input){
        //input[row][i] = parseInt(input[row][i]);
    //}
    //return input;
//}
...
//for(row in input){
    //total.push(input[row].reduce((a,b) => a + b, 0))
//}
